...
function PBHUD_OnEvent(self,event,...)
  ...
  if (event=="CHAT_MSG_PET_BATTLE_COMBAT_LOG") then
	for _,dmg,_,petname in string.gmatch(narg[0],"(.+)dealt (%d+) damage to your (.+)|t(.+) (%(+)(.+).") do sml_dprint(PBHUD_db.Debug,"PBHUD","SPECIAL ATTACK") PBHUD_PetDamage(petname,dmg) return end
	for _,dmg,_,petname in string.gmatch(narg[0],"(.+)dealt (%d+) damage to your (.+)|t(.+).") do PBHUD_PetDamage(petname,dmg) end
	for _,heal,_,petname in string.gmatch(narg[0],"(.+)healed (%d+) damage from your (.+)|t(.+).") do PBHUD_PetHeal(petname,heal) end
  end
  ------------------------------------------------------------------------
...
function PBHUD_PetDamage(petname,dmg)
  ...
  if(PBHUD_db.PetTeam[petslot].hp<0) then PBHUD_db.PetTeam[petslot].hp=0 end
  PBHUD_UpdatePetHUD()
end
------------------------------------------------------------------------
...
function PBHUD_CommandHandler(msg)
  ...
  if(msg=="debug") then sml_toggle(PBHUD_db.Debug) end
